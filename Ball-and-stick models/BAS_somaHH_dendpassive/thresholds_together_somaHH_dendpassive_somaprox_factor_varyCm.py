...
# Bug-prone, so harcoded:
I_all  = [[0.09,0.091,0.092,0.093,0.094,0.095,0.096,0.097,0.098,0.099,0.1,0.101,0.102,0.103,0.104,0.105,0.106,0.107,0.108,0.109,0.11],[0.1,0.101,0.102,0.103,0.104,0.105,0.106,0.107,0.108,0.109,0.11],[0.1,0.101,0.102,0.103,0.104,0.105,0.106,0.107,0.108,0.109,0.11],[0.1,0.101,0.102,0.103,0.104,0.105,0.106,0.107,0.108,0.109,0.11],[0.107,0.108,0.109,0.11,0.111,0.112,0.113,0.114,0.115,0.116,0.117,0.118,0.119,0.12],[0.12,0.121,0.122,0.123,0.124,0.125,0.126,0.127,0.128,0.129,0.13]]
cm     = 1.0 
cmfacs = [0.5,1.0,1.25,1.5,2.0,3.0]
N      = len(cmfacs)

outfolder   = 'Results/IStim/Soma%i/dendlen%i/denddiam%i/' % (somasize,dendlen,denddiam) +folderstring
plotname    = outfolder + 'thresholds_bashhdpas_somaprox_varyfactor'+plotstring+'.png' 
outfilename = outfolder + 'thresholds_bashhdpas_somaprox_varyfactor'+plotstring+'.txt' 
...
for l in range(N):
    ...
    for i in range(NI):
        ...
        infilename = infolder+'basHHdpas_cmf'+str(cmfac)+'_idur%i_iamp'%idur+str(iamp)+'_Ra%i_vinit' %Ra+str(v_init)+changestring+'_V.txt' 
