...
dtexp = -8
cms = [0.5,1.0,1.25,1.5]#,2.0]#,3.0]#[0.5,1.0,1.75,3]#[0.5,1,1.5]
idur = 2000
idelay   = 100.0
afteri   = 100.0
tstart   = -500.
...
Is      = [0.131,0.132,0.133,0.134,0.135,0.136,0.137,0.138,0.139,0.14]#[0.145,0.146,0.147,0.148,0.149,0.15]#[0.186,0.187]#[0.181,0.182,0.183,0.184,0.185,0.186,0.187,0.188,0.189]#[0.13,0.131,0.132,0.133,0.134,0.135,0.136,0.137,0.138,0.139,0.14]
models  = [488462965]#[478513407,478513437,488462965] # [478513437,488462965] # 
Nmodels = len(models)
Ncms    = len(cms)
NI      = len(Is)
thresholds = np.zeros(Ncms)

plotname  = 'figures/threshold_Allen_somaprox_models%s.png' %str(models) 
outfilename  = 'figures/threshold_Allen_somaprox_models%s.txt' %str(models) 
...
for l in range(Ncms):
    ...
    for j in range(Nmodels):
        ...
        for i in range(NI):
            ...
            infilename = "figures/%i/current_idur%i_iamp" % (testmodel,idur) + str(iamp)+"/idur%i_iamp" % idur + str(iamp)+"_changecmf" + str(cm) + "_cmdend" + str(cm_dend) + "_cmaxon"+ str(cm_axon) + "_vinit"+str(v_init)+"_addedRa.txt"
